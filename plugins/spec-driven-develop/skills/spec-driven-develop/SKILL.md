...
9. **Phase files are the source of truth** for task completion. If MASTER.md counts disagree with phase file checkboxes, the phase files win.

10. **Lookback is allowed at phase boundaries**. At the start of Phase 2 and Phase 5, review prior phase outputs for gaps. If gaps are found, update the earlier documents and log the update. This is not "going back" — it's ensuring quality before moving forward.
